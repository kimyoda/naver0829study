...
public class PhotoUploadController {
	...
	public String uploadgo1(Model model, @RequestParam String title, @RequestParam MultipartFile upload, 
		...
		// 사진을 업로드할 배포된 프로젝트 내의경로의 위치를 구한다
		...
		String extName = filename.substring(dotIdx); // abc.jpg => .jpg을 얻는다
		
		// 실제 파일이 아닌 랜덤파일명으로 저장하기, 값을 넣을때마다 변경 됌
	...
	public String uploadgo2(Model model, HttpServletRequest request, 
			@RequestParam String title, @RequestParam ArrayList<MultipartFile> upload) {
		...
		for (MultipartFile multi:upload) {
			String f = multi.getOriginalFilename(); // 실제파일명
			// 실제 파일에서 확장자 분리
