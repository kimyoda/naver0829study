  		    ...
  		    success:function(res) {
  		    	...
  		    	$.each(res, function(idx, item) {
  		    		...
  		    		s+= 
  		    			`
  		    			<tr>
  		    			  <td><img src="\${imgsrc}" width=30 height = 30 class = "rounded-circle" hsapce = 5 border = 1>\${item.name}</td>
  		    			  <td>\${item.myid}</td>
  		    			  <td>\${item.hp}</td>
  		    			  <td>\${item.email}</td>
  		    			  <td>\${item.gaipday}</td>
...
<body>
  ...
  <div class="input-group" style = "width: 400px;">
    ...
    <input type = "text" class = "form-control" style = "margin-left: 10px;" id = "word" placeholder="검색값입력">
  	<button type = "button" class = "btn btn-success btn-sm" id = "btnsearch" style = "margin-left: 10px;">검색</button>
