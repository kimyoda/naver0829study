...
public class BoardController {
	...
	public String addBoard(@ModelAttribute BoardDto dto, @RequestParam int currentPage, @RequestParam List<MultipartFile> upload,
		...
		// 사진들 업로드
		// 사진 업로드를 안했을 경우 리스트의 첫데이터의 파일명이 빈문자열이 된다.	
		// 즉 업로드했을경우에만 db에 저장을 한다
		if (!upload.get(0).getOriginalFilename().equals("")) {
			for (MultipartFile multi:upload) {
				...
				try {
					...
					// 파일은 따로 db에 insert 한다
					BoardFileDto fdto = new BoardFileDto();
					fdto.setNum(dto.getNum()); // boarddb에 방금 insert 된 num값
					fdto.setPhotoname(fileName);

					boarFileService.insertPhoto(fdto);
		...
		// 새글인 경우에는 1페이지로, 답글인 경우는 보던 페이지로 이동한다
