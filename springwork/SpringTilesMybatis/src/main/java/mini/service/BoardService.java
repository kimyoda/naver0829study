...
public class BoardService {
	...
	public void insertBoard(BoardDto dto) {
	  int num = dto.getNum(); // 새글일 경우, 0, 답글일 경우 0 보다 큰값이 들어있다
	  ...
	  if (num == 0) {
		  ...
		  // 답글인 경우 그룹은 그래도 사용하므로 regroup은 따로 안구한다
		  // 답글인 경우
		  this.updateRestep(regroup, restep); // 같은 그룹 중 전달받은 restep보다 큰 값이 있는글들은 모두 +1을 해준다
		  // 그리고 나서 각각의 값에 1을 더한다
		  restep = restep + 1;
		  relevel = relevel + 1;
	  }
	  
	  // 각각 새로 구입한 값들을 다시 dto 에 담는다
