...
class SubC extends SuperC {
	...
	SubC() {
		//super();//부모의 디폴트 생성자 호출 - 숨겨져있다(생략가능 - 단 디폴트인 경우만)
...
//	} //위의 방법보다는 아래의 방법으로 부모 생성자를 호출하는 경우가 많다
	SubC(String name, int age, String hp) {
		super(name,age);//부모의 2번째 생성자 호출된다, 생략이 안된다. 반드시 첫줄에 출력하여야 한다.
		System.out.println("sub의 세번째 생성자 호출");
		this.hp = hp;
	}
	
	public void dataShow() {
		//부모클래스의 멤버변수가 protected 이므로 패키지 상관없이 접근 가능하다
//		System.out.println("이름: "+this.name);
//		System.out.println("나이: "+this.age);
		System.out.println("이름: "+super.name);//부모가 가진 변수는 this, super 모두 가능하다
...
public class InheriConst {
	...
	public static void main(String[] args) {
		...
		sub2.dataShow();
		SubC sub3 = new SubC("캔디", 12, "02-455-788");//부모 디폴트에서 홍길동에서 불렀지만, 추후 캔디로 입력을 다시했다?
		sub3.dataShow();
	}
